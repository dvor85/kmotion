...
class Events:
    ...
    def get_prev_instances(self):
        try:
            stdout = subprocess.check_output(['pgrep', '-f', f"^python.+{Path(__file__).name} {self.feed}.*"], shell=False, text=True).splitlines()
            return [pid for pid in stdout if Path('/proc', pid).is_dir() and int(pid) != os.getpid()]
        except Exception:
            return []
