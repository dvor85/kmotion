...
class Kmotion_Hkd1(Process):
    ...
    def __init__(self, kmotion_dir):
        ...
        self.daemon = True
        self.name = 'hkd1'
        self.archive_dir = ''  # the 'root' directory of the images dbase
    ...
    def read_config(self):
        ...
        self.archive_limit_perc = config_main['archive_limit_perc']
        ...
        while self.active:
            try:
                ...
                while self.sleep(15 * 60):
                    du = shutil.disk_usage(self.archive_dir)
                    used_storage_perc = du.used * 100 // du.total
                    _cur_day = time.strftime('%Y%m%d')
                    log.info(f'used of storage {self.archive_dir} = {used_storage_perc}%')
                    ...
                    if self.today_dir.is_dir():
                        ...
                        for f in today_list:
                            if not f.name == _cur_day:
                                log.info(f'Move {f.name} from {self.today_dir} to {self.archive_dir}')
                                may_rmdir = True
                                for s in f.rglob('*'):
                                    if s.is_file():
                                        _d = self.archive_dir / s.relative_to(self.today_dir)
                    ...
                    if used_storage_perc > self.archive_limit_perc:
                        log.warn('archive limit reached')

                        for fulld in sorted(self.archive_dir.iterdir()) + today_list:
                            try:
                                if fulld.is_dir():
                                    d = fulld.name
                                    if _cur_day == d:
                                        log.error("** CRITICAL ERROR ** crash - delete todays data, 'archive' is too small")
                                        www_logs.error("Deleting todays data, 'archive' is too small")
                                    if not d.startswith('.'):
