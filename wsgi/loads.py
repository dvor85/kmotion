    ...
    def __call__(self):
        ...
        try:
            if self.config['misc']['config_enabled']:
                data['uname'] = subprocess.check_output(['uname', '-srvo'], shell=False, text=True).strip()
                data['cpu'] = 0
                ...
                vmstat = subprocess.check_output(['vmstat', '-s'], shell=False, text=True).splitlines()
                ...
                vmstat_a = subprocess.check_output(['vmstat'], shell=False, text=True).splitlines()[1:]
                vmstat_d = dict((k, v) for (k, v) in zip(vmstat_a[0].split(), vmstat_a[1].split()))
                data['cpuusage'] = dict(ci=vmstat_d["wa"],
                                        cu=vmstat_d["sy"],
                                        cs=vmstat_d["us"])

                dfout = subprocess.check_output(['df', '-h', self.archive_dir], shell=False, text=True).splitlines()[1].split()
                data['fsarch'] = dfout[2:]
                dfout = subprocess.check_output(['df', '-h', self.ramdisk_dir], shell=False, text=True).splitlines()[1].split()
