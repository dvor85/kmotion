...
def init_rcs(kmotion_dir, ramdisk_dir):
    ...
    mutex_kmotion_parser_wr(parser)
...
def update_rcs(kmotion_dir, ramdisk_dir):
    ...
    #logger.log('update_rcs() - Setting ramdisk_dir to \'%s\' in kmotion_rc' % ramdisk_dir, 'DEBUG')
    parser = mutex_kmotion_parser_rd(kmotion_dir)
    #parser.set('dirs', 'ramdisk_dir', ramdisk_dir )
    max_feed = parser.getint('misc', 'max_feed')
    version = parser.get('version', 'string')
    title = parser.get('version', 'title')
    ...
    config_js = '%s/www/www/js/config.js' % kmotion_dir
    f_obj = open(config_js, 'w')
    print >> f_obj, 'var max_feed=%d;' % max_feed
    print >> f_obj, 'var version="%s";' % version
    print >> f_obj, 'var title="%s";' % title
    print >> f_obj, 'var ramdisk_dir="%s";' % ramdisk_dir
    f_obj.close()
    ...
    parser = mutex_www_parser_rd(kmotion_dir)
    for func_num in range(1, max_feed):
...
def init_ramdisk_dir(kmotion_dir):
    ...
    parser = mutex_kmotion_parser_rd(kmotion_dir)
    ramdisk_dirs = parser.get('dirs', 'ramdisk_dir').split()
    max_feed = parser.getint('misc', 'max_feed')
    
    for ramdisk_dir in ramdisk_dirs:
	if not os.path.isdir('%s/states' % ramdisk_dir):
	    try:
		os.makedirs('%s/states' % ramdisk_dir)
		logger.log('init_ramdisk_dir() - creating \'states\' folder', 'DEBUG')
	    except OSError:
		pass
	
	for sfile in os.listdir('%s/states' % ramdisk_dir):
	    try:
		state_file='%s/states/%s' % (ramdisk_dir, sfile)
	        if os.path.isfile(state_file):
		    os.remove(state_file)
		    logger.log('init_ramdisk_dir() - deleting \'%s\' file' % (state_file), 'DEBUG')
	    except OSError:
	        pass
        ...
        for efile in os.listdir('%s/events' % ramdisk_dir):
	    event_file='%s/states/%s' % (ramdisk_dir, efile)
	    if os.path.isfile(event_file):
		try:
		    f_obj = open(event_file, 'r')
		    pid = f_obj.read()
		    pid = pid.strip()
		    f_obj.close()
		    if pid == '':
		        os.remove(event_file)
		        logger.log('init_ramdisk_dir() - deleting \'%s\' file' % (event_file), 'DEBUG')
        	except OSError:
            	    pass
