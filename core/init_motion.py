...
def gen_motion_configs(kmotion_dir):
	...
	if not parser.has_section('motion_feed%02i' % feed):
	    parser.add_section('motion_feed%02i' % feed)
	    parser.set('motion_feed%02i' % feed,'feed_enabled','false')
	        
    feed_list = [feed for feed in range(1, max_feed)
                 if parser.getboolean('motion_feed%02i' % feed, 'feed_enabled')]
...
#ffmpeg_bps 400000'''
        ...
        # pal or ntsc,
        if parser.getboolean('motion_feed%02i' % feed, 'feed_pal'):
            print >> f_obj1, 'norm 0' 
        else:
            print >> f_obj1, 'norm 1' 
        ...
        # framerate,
        if (parser.getboolean('motion_feed%02i' % feed, 'feed_smovie_enabled') or 
            parser.getboolean('motion_feed%02i' % feed, 'feed_movie_enabled')):
...
'''
        ...
        # show motion box
        if parser.getboolean('motion_feed%02i' % feed, 'feed_show_box'): 
            print >> f_obj1, 'locate on'
             
        # ptz enabled, if 'ptz_track_type' == 9, useing plugins, disable here
        if parser.getboolean('motion_feed%02i' % feed, 'ptz_enabled') and parser.getint('motion_feed%02i' % feed, 'ptz_track_type') < 9: 
        ...
        if (parser.getboolean('motion_feed%02i' % feed, 'feed_smovie_enabled')):
    	    ...
    	    print >> f_obj1, 'jpeg_filename %s%s/%%Y%%m%%d/%0.2d/snap/%%H%%M%%S' % (rel_prefix, images_dbase_dir, feed)

        # movie mode
        if parser.getboolean('motion_feed%02i' % feed, 'feed_movie_enabled'): 
        ...
        print >> f_obj1, 'on_event_start %s/core/event_start.sh %%t' % (kmotion_dir)
        print >> f_obj1, 'on_event_end %s/core/event_end.sh %%t' % (kmotion_dir)
        print >> f_obj1, 'on_camera_lost %s/core/camera_lost.sh %%t' % (kmotion_dir)
        #print >> f_obj1, 'on_picture_save ln -sf %%f %s/%0.2d/last.jpg' % (ramdisk_dir, feed)
        print >> f_obj1, 'on_picture_save %s/core/picture_save.sh %%f' % (kmotion_dir)
